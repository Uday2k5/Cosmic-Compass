...
// 1. Data Definitions (60 days of asteroid coordinates in KM)
const REAL_INPUT_DATA_KM = [
    [-95139447.20078547, -211765941.0111033, -37866005.87452585],
    [-93566359.59098476, -212942693.1369387, -37744247.2834471],
    [-91986186.39721616, -214103320.6226206, -37619630.47576535], 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sun = {
        x: [0], y: [0], z: [0],
        mode: 'markers', type: 'scatter3d', name: 'Sun',
        marker: { color: 'yellow', size: 10, symbol: 'circle' }
    };

    const earthOrbit = {
        ...generateOrbit(1), 
        mode: 'lines', type: 'scatter3d', name: 'Earth Orbit',
        line: { color: 'blue', width: 2 }
    };

    const marsOrbit = {
        ...generateOrbit(1.52), 
        mode: 'lines', type: 'scatter3d', name: 'Mars Orbit',
        line: { color: 'red', width: 2 }
    };

    const erosKnown = {
        ...getKnownErosPath(),
        mode: 'lines', type: 'scatter3d', name: 'Eros (Known Path)',
        line: { color: 'cyan', width: 4 } 
    };
    ...
    const layout = {
        ...
        margin: { l: 0, r: 0, b: 0, t: 40 },
        scene: {
            xaxis: { title: 'X (AU)', gridcolor: 'rgba(255,255,255,0.1)' },
            yaxis: { title: 'Y (AU)', gridcolor: 'rgba(255,255,255,0.1)' },
            zaxis: { title: 'Z (AU)', gridcolor: 'rgba(255,255,255,0.1)' },
            bgcolor: '#000000',
            aspectmode: 'data'
        },
        paper_bgcolor: 'rgba(0,0,0,0)',
    ...
    Plotly.newPlot(plotDiv, [sun, earthOrbit, marsOrbit, erosKnown], layout);

    // API Prediction Logic
    async function fetchPrediction() {
        console.log("Calling API at https://cosmic-compass-z0gt.onrender.com/predict");
        const apiUrl = 'https://cosmic-compass-z0gt.onrender.com/predict';
        ...
        try {
            ...
            const path = data.prediction;
            return {
                x: path.map(row => row[0] / KM_TO_AU),
                y: path.map(row => row[1] / KM_TO_AU),
                z: path.map(row => row[2] / KM_TO_AU),
                mode: 'lines+markers',
                type: 'scatter3d',
                name: 'Eros (Predicted Path)',
                line: { color: 'orange', width: 5, dash: 'dash' },
                marker: { size: 4, color: 'orange' }
            };

        } catch (error) {
            console.error('Prediction Failed:', error);
            alert(`Error: ${error.message}\n\nNote: If this is the first load, Render may take up to 60 seconds to "wake up".`);
            return null; 
        }
    }
    ...
    predictButton.addEventListener('click', async () => {
        predictButton.disabled = true;
        predictButton.textContent = 'Calculating Trajectory...';
        
        const trace = await fetchPrediction();
        if (trace) {
            Plotly.addTraces(plotDiv, [trace]);
            predictButton.textContent = 'Prediction Added';
        } else {
            predictButton.textContent = 'Predict Future Path';
            predictButton.disabled = false;
        }
